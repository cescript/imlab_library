...
int encoder_parameters(uint32_t width, uint32_t height, char *options, struct encoder_parameters_t *out) {
    out->type = 0;
    out->width = width;
    out->height = height;

    if(out->type == 0) {
        out->feature_size = out->width * out->height;
    } else {
        out->feature_size = 0;
    }
...
struct feature_t *encoder_create(int width, int height, char options[]) {
    ...
    // fill the parameters
    model->feature_size = encoder_parameters(width, height, options, model->parameters);
